      ...
      <div class="share">
        <svg @click="sharet" t="1704349617698" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
          ...
          <path
            ...
            fill="#2c2c2c" p-id="2537"></path>
        </svg>
        <p>分享</p>
        <input type="text" class="input" placeholder="搜索壁纸">
      </div>
    </div>
    <div class="boxe">
      <div class="foter" >
        ...
        <div class="Tags" v-for="(item, index) in Label" :key="index">
          <img :src= item.bgimg >
          <h3>{{ item.name }}</h3>
        </div>
      </div>
      <div class="Popular">
        ...
        <div class="imagest" v-for="(item, index) in popular" :key="index">
          <img :src= item.image >
          <h3>{{ item.names }}</h3>
        </div>
      </div>
...
<script>
  ...
  export default{
    ...
    mounted(){
      axios.get('http://127.0.0.1:3000/wall/label')
      .then((response) => {
